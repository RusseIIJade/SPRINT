...
document.addEventListener("DOMContentLoaded", () => {			
	...
	//If it's an invalid shows an invalid notice
	loginForm.addEventListener("submit", e => {
		e.preventDefault();

		setFormNotice(loginForm, "invalid", "Invalid Username or Password");
    });
	...
	document.querySelectorAll(".form__input").forEach(inputElement => {
		inputElement.addEventListener("blur", e => {
			if (e.target.id === "signupPassword" && e.target.value.length > 0 && e.target.value.length < 8) {
				setInputinvalid(inputElement, "Username must be at least 8 characters in length");
			}
		});

		//This clears invalid by default when the user types inside of a placeholder or inputs field
		inputElement.addEventListener("input", e => {
			clearInputInvalid(inputElement);
		});
	});
